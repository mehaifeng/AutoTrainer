...
class EfficientNetConverter(BaseConverter):
    """EfficientNet系列模型转换器"""
    ...
    def load_model(self) -> nn.Module:
        ...
        Returns:
            加载的模型
        """
        self.logger.info(f"加载EfficientNet模型: {self.model_name}")
        ...
        if hasattr(model, 'classifier'):
            ...
            raise ValueError("模型没有classifier属性")
        
        # 加载权重
        model.load_state_dict(state_dict)
        
        self.logger.info("✓ EfficientNet模型加载成功")
        return model
    ...
    def get_input_size(self) -> int:
        ...
        return size_map.get(self.model_name, 224)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='EfficientNet模型转换')
    ...
    # 创建转换器
    converter = EfficientNetConverter(config)
